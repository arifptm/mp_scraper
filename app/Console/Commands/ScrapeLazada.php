...
class ScrapeLazada extends Command
{    
    protected $signature = 'sc:lz';
    protected $description = 'Scraping Lazada';
    ...
    public function handle(){
	     ...
	     *
	     */   
	    if ($crawler->filter('#pdtsku')){
	    	$sku = explode('-',$crawler->filter('#pdtsku')->text())[0];
	    	$url_sku = "https://www.lazada.co.id/ajax/ask/questionsSegmented?sku=".$sku;
	    	$seller_json = json_decode(file_get_contents($url_sku));
        	
        	$seller['name'] = $seller_json->data->pdpSeller->name;        	
        	$seller['image_url'] = $seller_json->data->pdpSeller->image;        
        	$seller_id = $seller_json->data->pdpSeller->id;
	    	...
	    	$seller['name'] = 'Lazada';
	    	$seller['image_url'] = 'http://static.99toko.com/default_shop.png';
	    	$city['name'] = 'Jakarta';
	    }
